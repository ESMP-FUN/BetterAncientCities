...
/**
 * `/acp` admin command. Fallback to the GUI (`/ancient menu`, added later); for now
...
class BacCommand(private val plugin: BetterAncientCities) : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if (!plugin.isReady) {
            sender.sendMessage("§cBetter Ancient Cities is still starting up.")
            return true
        }
        when (args.getOrNull(0)?.lowercase()) {
            ...
            "reload" -> { plugin.reloadConfig(); sender.sendMessage("§aBetter Ancient Cities config reloaded.") }
            ...
            else -> sender.sendMessage("§cUnknown subcommand. §7Try §f/ancient help§7.")
        }
        return true
    }

    private fun openMenu(sender: CommandSender) {
        val player = sender as? Player ?: run { sender.sendMessage("§cThe menu is players-only. Try §f/ancient list§c."); return }
        plugin.menuService.openCityList(player)
    }

    private fun sendHelp(sender: CommandSender) {
        sender.sendMessage("§5§lBetter Ancient Cities §7— admin commands")
        sender.sendMessage("§f/ancient menu §7— open the admin GUI")
        sender.sendMessage("§f/ancient list §7— list discovered cities (approved + pending)")
        sender.sendMessage("§f/ancient info <id> §7— details of a city")
        sender.sendMessage("§f/ancient approve <id> §7— activate a pending city")
        sender.sendMessage("§f/ancient delete <id> §7— unregister a city")
        sender.sendMessage("§f/ancient tp <id> §7— teleport to a city")
        sender.sendMessage("§f/ancient check §7— is the block you're looking at protected? (ignores your bypass)")
        sender.sendMessage("§f/ancient snapshot <id> §7— capture the city's structure for restoration")
        sender.sendMessage("§f/ancient reset <id> §7— restore the city from its snapshot")
        sender.sendMessage("§f/ancient reload §7— reload config.yml")
        sender.sendMessage("§f/ancient ban <id> <player> [reason] §7— loot-ban a player from a city")
        sender.sendMessage("§f/ancient unban <id> <player> §7— lift a loot ban")
        sender.sendMessage("§f/ancient bans <id> §7— list a city's loot bans")
        sender.sendMessage("§f/ancient resetloot <id> <player> §7— let a player loot the city fresh")
    }
        ...
        for (c in cities) {
            ...
            sender.sendMessage(mm.deserialize(
                "<gray>#<white>${c.id} <gray>[$tag<gray>] <white>${c.world} " +
                    ...
                    "<click:run_command:'/ancient open ${c.id}'><hover:show_text:'<gray>Open city <white>#${c.id}<gray> in the GUI'>" +
    ...
    private fun handleCheck(sender: CommandSender) {
        ...
        sender.sendMessage("§5/ancient check §7— §f${block.type.name.lowercase()} §7at §f${loc.blockX}, ${loc.blockY}, ${loc.blockZ}")
        ...
        when {
            inPiece && city != null && !city.approved ->
                sender.sendMessage("§e→ City is pending approval, so protection isn't active yet. Approve it with §f/ancient approve ${city.id}§e.")
    ...
    private fun handleReset(sender: CommandSender, idArg: String?) {
        ...
        if (!plugin.snapshotManager.hasSnapshot(city.id)) {
            sender.sendMessage("§cCity #${city.id} has no snapshot. Capture one first with §f/ancient snapshot ${city.id}§c.")
            return
        }
    ...
    private fun resolve(sender: CommandSender, idArg: String?): City? {
        val id = idArg?.toIntOrNull() ?: run {
            sender.sendMessage("§cUsage: provide a numeric city id (see §f/ancient list§c).")
            return null
        }
